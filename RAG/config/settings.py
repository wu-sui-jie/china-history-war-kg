...
# 凭证撤销查询的失败策略取值。与 server/introspection.py 的常量同值，
...
def _first_env(*keys: str, default: str = "") -> str:
    ...
    `RAG-command` 这类连字符名，而连字符在 Linux 的 shell 里无法 export）。
...
class Settings:
    ...
    # 文本检索模式：keyword / vector / hybrid —— 部署级全局开关
    # 兜底值与 defaults 对齐 hybrid/rrf（环境变量缺项时不会静默落到别的模式）
    ...
    # 关键词检索用词上限（集中在此声明，由 server/text/searcher.py 读取）
    ...
    # 送入 F06 的融合证据条数上限（默认 18；调小可压制推理长度/截断与首延迟）
    ...
    # 那种"隐式生产"只告警；显式写了才当作生产档，触发 CORS 等启动门禁。
    require_active_version_explicit: bool = False
    # 版本来源声明：由启动方写入，让 health 能区分
    ...
    # 鉴权模式。把"谁来把关身份"这件事显式化——单个布尔开关无法区分
    # "由 nginx 的 auth_basic 把关"与"根本没人在把关"，两者在配置里长得一模一样，
    # 运维看不出自己属于哪一种：
    ...
    # token 的来源与受众。与签发端 backend/jwt_util.py 的取值必须一致，
    ...
    # 生产档下**必须显式选择撤销策略**。
    #
    # 原因："两个值都不填"等于隐式接受"停用账号 / 改密码后旧 token 在自然过期前
    ...
    # 显式确认"就是要公开 API"：生产 + wildcard CORS 时
    ...
    # 否则会出现"外部调用还没返回、服务端已经按 deadline 收流"的现象
    ...
    def cors_startup_problem(self) -> Optional[str]:
        """**显式生产档** + 通配 CORS 且未显式确认时返回错误说明，否则 None。
    ...
    def auth_startup_problem(self) -> Optional[str]:
        ...
        2. **生产档 + disabled**：显式生产档下 `RAG_AUTH_MODE=disabled` 意味着
           "知道 /rag/ 地址的人都能调用问答接口"，既不受旧系统角色控制，也消耗模型配额与
           限流额度。这种情况只打一条 WARNING 不够——WARNING 会被忽略，没配的人根本不会
           去看日志；因此改为启动即失败，强制部署方**显式做一次选择**：`jwt` 或 `nginx`。
        ...
        # 说的是"验签交给网关"；若让新模式静默覆盖旧开关（require_auth=False），
        # 就变成"两边都不拦"：nginx 那边若没配 auth_basic，没有任何人在把关，
        # 而配置看起来是写了的。
    ...
    def validate(self) -> None:
        ...
        if self.bot_api_key_explicit and not self.bot_api_key:
            ...
            # 与 CORS_ALLOW_ORIGINS 的坑同源：漏配必须报错，
        ...
        if not self.cors_allow_origins:
            # 显式写空（CORS_ALLOW_ORIGINS=）若被 `or ["*"]` 兜成通配符，
            # 等于"漏配保护"反过来成了"放开保护"：漏配必须报错
            problems.append(
                "CORS_ALLOW_ORIGINS 不能为空：留空会失去跨域白名单语义。"
                "开发用 `*`，生产填站点域名（逗号分隔）；两者都要显式写出"
            )

        # 外部调用超时预算必须**严格小于** SSE 总上限，并留出收尾余量：
...
def get_settings() -> Settings:
    # 鉴权模式的解析：新配置 RAG_AUTH_MODE 优先，旧开关
    ...
    settings = Settings(
        ...
        # 凭证撤销查询。密钥名接受两个：RAG_INTERNAL_SERVICE_KEY 是本服务
        ...
        # 两个开关都接受；`RAG_REQUIRE_REVOCATION_CHECK` 只是"要撤销查询"的
