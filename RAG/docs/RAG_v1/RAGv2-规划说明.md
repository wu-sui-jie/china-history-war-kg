...
- 阶段：RAGv2
- 状态：✅ 已执行（本文件为预开发规划存档；完成情况见 [RAGv2-在线问答链路.md](RAGv2-在线问答链路.md)）
- 前置：RAGv1 已完成（见 [RAGv1-离线数据链路.md](RAGv1-离线数据链路.md)）
- 目标：让用户能**输入问题 → 收到带引用与过程事件的流式回答**（F02→F06），打通在线问答主链路

> 说明：本文件保留为"规划 → 实际"对照存档。实施中对少数点做了细化/裁剪，
> 偏差已在 [RAGv2-在线问答链路.md](RAGv2-在线问答链路.md)"四、数据特征发现"与
> "五、边界与未做事项"如实记录（例如：different_object 仅对 exact 单值组生效、
> F02 LLM 兜底默认关闭、F06 无 key 时用离线摘要回答器、answer 流式按句模拟等）。
