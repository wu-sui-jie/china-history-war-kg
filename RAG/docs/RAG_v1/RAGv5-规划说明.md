...
| 3 | **文本模式开关形态**：部署级全局开关（开发说明推荐）vs 按请求可切（决定缓存键形态与请求契约是否新增字段） | ✅ **部署级全局开关** `TEXT_MODE`（v5 期间仓库默认 keyword、演示 `.env` 设 hybrid；**2026-09-15 审核整改后代码级默认已对齐 hybrid/rrf**）；缓存键已含 mode（C3）；请求契约不新增字段 |
...
2. **M1 向量线**：✅ **已完成主体（2026-09-13）**——百炼 embedding 接线（构建侧，含小样维度验证）→
   ...
   → 融合策略定档 `rrf`；剩余：`.env` 已切 `TEXT_MODE=hybrid`（仓库默认 2026-09-15 审核整改后
   亦对齐 hybrid/rrf），演示示例题选型与向量通道的提示词调优归入 M3/M5；
