...
> 带 token（`frontend/src/api/authToken.ts` 保管并拼进请求头）。第 13 轮整改把原先单一的
> 布尔开关换成三档 `jwt` / `nginx` / `disabled`，因为"nginx 在把关"与"根本没人在把关"
> 在配置里长得一样；**显式生产档下 `disabled` 会拒绝启动**（原先只打一条会被忽略的 WARNING），
> `nginx` 档若监听非回环地址也拒绝启动。**生产模板默认已改为 `jwt`**（`deploy/env/rag.env`）：
> 原模板写 `nginx`，而 nginx 侧的 `auth_basic` 是注释状态，照着两份模板部署会得到
...
| 首屏体积门禁 | `npm run check:bundle` | 通过（入口 gzip 25.5 kB、vendor 75.9 kB、首屏合计 101.4 kB，上限 190 kB）。**2026-09-25 修复**：该门禁原先把资源前缀写死成 `/assets/`，而并入模式产物引用的是 `/rag/assets/`——正则一条都匹配不上、三个体积全是 0 却照样报通过。现改为按产物自解析前缀，且**找不到入口或 vendor 直接失败**，standalone 与 integration 两种产物各有用例（第 12 轮审查 P2-3） |
| 文档与配置一致性 | `python scripts/check_docs.py --strict` | 通过（相对链接、current 口径、`.env.example`、**数据计数与清单一致**） |
| 密钥扫描 | `python scripts/check_secrets.py` | 未发现明文密钥。**范围已扩到全仓库**（`--root .`，CI 由 `secret-scan` job 执行）：脚本原先的根目录写死为 `RAG/`，旧模块 CI 的 grep 又只扫 `backend` 与 `entity-event-relation`，于是 `deploy/`、`feishu-bot/`、工作流与根级脚本全在门禁之外——`deploy/env/backend.env` 里的固定口令与 JWT 密钥正是从这条缝漏过去的（第 12 轮审查 P0-2 / P2-8）。已用复刻当年内容的样本验证：现在会命中那两行，占位符样本不误报 |
| 静态检查（RAG） | `python -m ruff check server config contracts lib data scripts evaluation tests` | All checks passed（第 12 轮审查 P2-9：名为 `lint` 的 CI job 此前**没有执行 Ruff**，本次修掉审查发现的 7 项——6 个无占位 f-string、1 个导入位置——并把该步骤固化进 CI，ruff 版本钉死 0.14.4） |
