/** 首屏体积门禁的用例。
 *
 * `scripts/check-bundle.mjs` 必须按产物里真实的资源前缀解析入口/vendor chunk：
 * 前缀写死成 `/assets/` 时，`npm run build:integration` 的产物引用 `/rag/assets/...`，
 * 正则一条都匹配不上，于是打出"入口 0.0 kB / vendor 0.0 kB / 首屏合计 0.0 kB，体积门禁通过"。
 * 一个永远为真的门禁比没有门禁更糟：它会让真实回归也显示为通过，
 * 所以"解析不到入口"必须判失败，不能按 0 通过。
 *
