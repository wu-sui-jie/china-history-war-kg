...
数据版本：`--version` 会写入环境变量 RAG_ACTIVE_VERSION，由 app 的 lifespan 读取并
显式加载——只把这个参数用于启动自检打印不够，uvicorn 重新导入模块后 API 会按
"最新目录"选版本，两者可能不是同一份数据。

版本来源：传了 `--version` 时同时写 RAG_VERSION_SOURCE=cli_explicit，否则 health 只能
看到"环境变量被固定"（env_pinned），无法区分"命令行显式指定"与".env 固定"。
"""
...
def pin_version(version: str | None) -> str:
    ...
    按字符串导入模块，晚一步就会出现"启动日志说 A、API 实际用 B"；
    ...
    否则"没传 CLI 参数、实际按环境/扫描选版"的运行会被 health 错标成"CLI 显式固定"，
    而这种错标恰好出现在最需要准确诊断的场合。
    """
    if not version:
