"""凭证撤销查询器（方案 B：Token Introspection）。
...
def test_失败结论按失败策略得出(monkeypatch):
    """失败时返回的是**策略推导出的结论**，并标记 `unavailable`。

    标记的意义：调用方要能区分"后端说这张凭证已失效"（401）与"我们问不到后端"
    （503）——两者都回 401「登录已失效」时，后端抖一下就把全体用户"登出"了。
    """
    client = _client(fail_mode="closed")
...
# ---------------------------------------------- 撤销策略必须显式选择
#
# 问题："两个值都不填"等于隐式接受"停用账号 / 改密码后旧 token 在自然过期前
...
def test_同_token_并发只查一次后端(monkeypatch):
    """single-flight：TTL 到期瞬间的并发请求不该各发一次。"""
