"""服务间内部接口（第 13 轮复核，文档第四节方案 B：Token Introspection）。

## 为什么需要

第 13 轮让旧后端做到了"改密码 / 封号后旧 token 立刻失效"——`before_request` 每次都
...
1. **未配置密钥 = 接口不可用（503），而不是"不校验"**。这是本仓库反复出现的
   一类缺陷：可选的保护开关漏配时静默变成"没有保护"。一个匿名的 introspection
...
# 内部接口的统一前缀。**app.py 的全局鉴权按此前缀放行**（这些请求没有用户 token），
