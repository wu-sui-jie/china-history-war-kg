...
`/api/login` 与 `/api/sign_in` 必须节流：没有它，一个脚本可以对某个账号每秒试上千次密码，
...
## 时钟口径：落库一律用 UTC epoch

落库的时间不能取 `time.monotonic()`：它是"本机自启动以来经过的秒数"，
只在**同一次开机**内可比：
...
落库的时间**一律是 `time.time()`（UTC epoch 秒）**，字段名带上 `_epoch` 后缀把单位
...
"SELECT failures → Python 里 +1 → UPDATE"会丢计数：两个并发请求同时读到 3，
然后都写回 4——实际发生两次失败，库里只加了一次。爆破脚本要的正是"用并发把计数做废"。
因此计数、窗口滚动与锁定判定全部放进**一条 UPSERT** 的 SQL 里
（`_UPSERT_ATTEMPT`）：`ON CONFLICT ... DO UPDATE` 的 SET 表达式读的是该行的旧值，
因此整条语句是一次原子读改写，不会丢计数。Python 侧只剩两个参数（now 与阈值）。

## 限流自身故障时不能静默失效

限流表读写失败时"记一条日志、继续放行"是不行的：数据库锁冲突、表损坏或迁移失败都会让
爆破保护**无声地消失**，而登录看起来一切正常。因此由 `LOGIN_GUARD_FAILURE_MODE` 显式选择：

| 取值 | 行为 | 适用 |
| --- | --- | --- |
| `closed`（默认） | 无法判定就拒绝本次登录：抛 `LoginGuardUnavailable`，路由回 **503** | 生产 |
| `open` | 记 `ERROR` 日志后放行（可用性优先） | 开发/内网 |
...
# 废弃的 monotonic 口径列名。存在这些列 = 表是按旧口径建的，整表作废：
