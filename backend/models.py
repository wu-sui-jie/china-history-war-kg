...
class UserInfo(db.Model):
    ...
    token_version / disabled 是"让还没过期的 token 提前失效"的两个开关（第 13 轮整改）：
...
# Neo4j 调用失败原先只在响应里带一句 sync_status=failed——没有重试、没有记录，
# 管理台列表与图谱从此长期不一致，而且**没人知道差在哪一条**（质检只能发现数量差）。
#
...
class Neo4jSyncJob(db.Model):
    ...
    后台 worker 会把它补上。原先的顺序是"先提交业务、再调 Neo4j、失败后才记一条"，
    提交后崩溃就会永久不一致且无人知晓。
    ...
    原先只有一种 "upsert"：更新失败重放时按新名字创建/合并，**可能留下旧节点并造出重复**，
    因为任务里既没有原 Neo4j id 也没有操作类型。现在这两样都记下来了。
