...
def clear_migration_tables():
    """
    清空迁移表数据（保留用户表 UserInfo）
    使用原生 sqlite3，支持 WAL 模式
    """
    import sqlite3
    import os
    ...
    try:
        ...
        tables_to_clear = [
            ...
            'event_event_relations', 'event_organization_rel',
            'event_place_relations', 'event_person_relations'
        ]
        ...
        cursor.execute("""
            ...
            WHERE name IN ({});
        """.format(','.join(['?' for _ in tables_to_clear])), tables_to_clear)

        conn.commit()
        print(f"✅ 已清空所有迁移表数据（UserInfo用户表保留）")
        return True, "数据已清空"
...
def get_migration_stats():
    ...
    tables = [
        ...
        ('organizations', '组织'),
        ('persons', '人物'),
        ('event_event_relations', '事件-事件关系'),
        ('event_organization_rel', '事件-组织关系'),
        ('event_place_relations', '事件-地点关系'),
        ('event_person_relations', '事件-人物关系')
    ]
