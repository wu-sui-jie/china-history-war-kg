"""`/api/extract/entities-events` 抽取链的**录制回放**回归（第 11 轮 B 类）。

**为什么需要这组用例。** 这个端点的行为此前没有任何回归保护：`tools/snapshot_responses.py`
刻意跳过它（"结果不确定，放进来只会制造噪音"），第 10 轮修掉的 bug（实体对象被当
place_list 渲染进提示词）正是从这儿漏过去的。三层防护各管一段，别指望一层包打天下：
