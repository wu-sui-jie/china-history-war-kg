"""登录限流、账号锁定与注册开关（文档第十一节）。

没有这三件事时：`/api/login` 与 `/api/sign_in` 谁都能以任意频率调用，
...
另有一组**时钟口径**用例（见下面 "落库时间" 一节）：限流状态是落库的，
...
# ---------------------------------------------- 并发计数与失败策略
#
# 计数必须是原子的："读 → Python 里 +1 → 写回"会让两个并发请求同时读到
# failures=3 后都写回 4——实际发生两次失败，库里只加了一次。爆破脚本要的就是这个。
#
# 限流自身故障时不能静默失效。"读不到限流状态就放行"等于让"把限流表弄坏"
