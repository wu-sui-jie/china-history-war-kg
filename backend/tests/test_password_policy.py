"""口令策略的唯一口径（第 13 轮复核整改 §2.1）。

整改前有两套数值：注册/改密码是 10–64，而首个管理员命令是 6–20。后果不是"数值不美观"，
而是**权限最高的账号允许最弱的密码**，同时 21–64 位的强口令反而设不上。

本文件钉四件事：

1. 边界：9 位拒绝、10 位与 64 位接受、65 位拒绝；
2. 首尾空白：明确报错（而不是静默去掉——那正是前端原先的行为）；
3. **登录路径不查强度**：提高策略前建的短口令账号必须还能登录；
...
def test_首尾空白报错而不是被静默去掉():
    """前端曾经 `password.trim()`——那是"静默改用户输入"，本模块的存在就是为了取代它。"""
