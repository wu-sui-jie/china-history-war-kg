...
的 tp/fp/fn 与 F1 不同"，但第 8 轮复核在**同一个进程里**跑两次，看到指标逐字段一致，
于是把它降级成"只有 error_samples 不可复现"。第 9 轮用两个不同 PYTHONHASHSEED 的
**子进程**复测，确认指标级漂移真实存在：
...
改变，tp/fp/fn 就跟着动。同一进程内跑两次看不到，是因为哈希种子相同（这正是第 8 轮
复核的盲点，**不是**因为不存在）。
