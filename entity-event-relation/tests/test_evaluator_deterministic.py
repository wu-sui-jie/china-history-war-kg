"""同输入两次评估必须给出同一份结果（评估可复现）。

**为什么需要这组用例。** 曾实测到"同一 pred / gold / config 连跑两次，关系抽取的
tp/fp/fn 与 F1 不同"，但"跑两个独立进程各一次看到一致"就被当成了没问题。问题在**样本量**：
漂移是"某些哈希种子下相似度并列的取舍不同"，两次完全可能碰巧落在同一个结果上
（三颗种子能给出 3 个不同 tp 值，只抽 2 次有约 1/3 概率抽到同一对），
而且那两次没有显式控制 `PYTHONHASHSEED`。用三颗**显式**种子复测，指标级漂移真实存在：
...
1. `test_greedy_matching_ignores_input_order`：直接对匹配函数喂不同顺序的同两个集合，
   断言结果逐字段一致。进程内、确定性，针对**指标级**漂移（不排序必失败，且失败信息直接
   指出是 tp 变了）。
...
# tp=1。不排序时把 gold 顺序反过来就能看到 tp 变化；排序后两种顺序必然一致。
_TIE_HEAD = "E1"
_TIE_RELATION = "指挥"
