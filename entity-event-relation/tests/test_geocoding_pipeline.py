"""日配额截断时，pipeline 必须让"只跑了一半"这件事可见。

编码层刻意保留"日配额耗尽即停整批"的行为（当天不会再成功，继续跑只是把剩余地点全刷成失败），
但**调用方**——审核与导入——看到的是一份部分结果。`run_full_pipeline` 拿到截断结果后若照常
往下走，"跑完了"的错觉就会带着半份结果入库。这里的用例钉住口径：
打醒目提示，且默认不进入审核 / 导入。
