"""C-4（EER-5）：提示词版本必须由源码哈希机械派生，不能靠人记得 bump。

修前：`PROMPT_VERSION = "prompt-v2-20260420"` 是手写的单一版本串，`PROMPT_VERSIONS` 五个
分项同日同版。改提示词忘了改版本 → ①缓存键不变，命中旧结果，改了等于没改；
②产物 metadata 里的 `prompt_version` 与实际提示词不符（学术评估里这是硬伤）。

修后：版本串 = 人写的语义版本 + `war_extraction/prompts/` 源码的 sha256 前 8 位。
