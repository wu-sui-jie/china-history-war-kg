"""
抽取编排的共享入口（第 11 轮 C-1，收口 EER-7）。

**为什么要有这个模块。** 此前"分段循环 + 三阶段调用 + 去重收集 + 失败诊断"有两份**平行实现**：
backend 的 `llm_pipeline.extract_all_optimized` 与离线链路的 `main.py:process_long_text`
/`process_single_file`。两份并存已经漂过一次——第 10 轮修的那个 bug（实体对象被当
`place_list` 传进提示词）就是平行实现的产物。现在编排只有这一份，两边各自的后处理
用钩子挂进来，结构上不会再漂。

**分工**（这也是本模块的"语义决定"，工作单 C-1 要求先定下来）：
...
1. **实体名列表**：只放非空名称、去掉首尾空白、段内重复保留（backend 口径，第 10 轮已按此对齐）。
   离线链路原先会把空名一起 join 进提示词（`geo_name` 为 `None` 时还会直接抛异常）。
2. **失败策略**：**按阶段继续**——某一段的某阶段失败只作废该阶段，其余阶段照跑，失败逐条进
   `partial_errors`、`stage_ok` 只给成功的阶段计数。离线链路原先是"一段里任一阶段失败就把整段
   三个结果全清空"，现在与 backend 一致（局部失败不整体作废，与第 6 轮审核 M1 的口径同源）。
...
4. **分段失败兜底**：切分抛异常时退化成"整篇一段"（backend 原口径），不让整个任务因切分崩掉。
...
def entities_from_dict(data: Dict) -> EntityExtractionResult:
    """把缓存里的字典还原成实体结果（与 main.py 原来的 dict_to_entities 同一实现）。"""
...
def events_from_dict(data: Dict) -> EventExtractionResult:
    """把缓存里的字典还原成事件结果（与 main.py 原来的 dict_to_events 同一实现）。"""
...
def relations_from_dict(data: Dict) -> RelationExtractionResult:
    """把缓存里的字典还原成关系结果（与 main.py 原来的 dict_to_relations 同一实现）。"""
...
def run_extraction(llm, text: str, *, splitter: Optional[TextSplitter] = None,
    ...
    for index, (start, end, chunk_text) in enumerate(chunks, 1):
        # 空白段直接跳过（backend 原口径）：给它跑三阶段只会烧额度、换回空结果
...
def _run_single_chunk(*, index: int, start: int, end: int, chunk_text: str,
    ...
    def fail(stage: str, exc: Exception) -> None:
        # 文案与 backend 原实现逐字一致（"[start-end] 阶段失败: exc"），界面上要能直接看
    ...
    if on_entities_ready:
        ...
        # 钩子返回后的名称列表才是关系阶段要用的那一份——两边原实现都是这个顺序。
