...
├── __init__.py                    # 包初始化
├── README.md                      # 本文件
├── db_path.py                     # 主库路径解析（--db-path / EER_DB_PATH / 默认仓库根）
...
> `geocoding_progress_*.jsonl`，那是跑批产物，已 gitignore、不入库。
...
> **包名口径**：本模块在 `war_extraction.geocoding` 之下（顶层 `src` 已改名为 `war_extraction`
> 并正式打包），早先文档里写的 `cd <项目根>/entity-event-relation/src` **那个目录已不存在**。
...
主库路径的解析顺序（收在 `war_extraction/geocoding/db_path.py`）：显式 `--db-path` /
...
被日配额截断时，`pipeline` **默认在编码后停下**，不进入审核 / 导入——否则"跑完了"的
...
**一个批次一个进度文件**：文件名的 `<run_id>` 是批次开始时的时间戳（精确到毫秒），
