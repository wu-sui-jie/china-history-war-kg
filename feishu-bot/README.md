...
RAG 探活与非流式接口是否存在 → `/help` → 主问题（占位卡 + 最终卡两段产物，是否出子图、
几个折叠区、哪些按钮）→ 追问（历史是否透传）→ 纠错反馈（工单卡片 + 落库）→
`/new` 重置（历史清空、feedback 保留）→ 事件去重。
...
2. 发 `介绍一下长平之战。` → 先收到「正在检索…」占位卡，几秒~二十几秒后同一张卡变成
   正文 + `引用（N）` 折叠区 + 示例问题按钮 + 「反馈有误」按钮（两段式回复）
3. 点开 `引用（N）` 看原文片段；有子图的问题（如 `介绍一下涿鹿之战。`）卡片里应有关系图**图片**
4. 点卡片底部的示例问题按钮 → 收到那条问题的新回答（验证按钮回调）
5. 追问 `他后来怎么样了？` → 回答能对应上一轮（验证多轮上下文）
6. 发 `/new` → 收到"已清空本会话的上下文"；再追问 `他后来怎么样了？` → 不再带上文
7. 把机器人拉进一个测试群（仅自己可见的应用**不能**加入外部群），群里 `@机器人 介绍一下赤壁之战。`
   → 同效；**不 @ 时机器人不应有任何反应**
8. 点「反馈有误」→ 立刻弹 toast「已收到反馈」；**Ctrl-C 停掉机器人再启动**，追问上文仍在（会话落 SQLite）
...
| 2 | 单聊问「介绍一下长平之战。」 | 先收到「正在检索…」占位卡，几秒~二十几秒后**同一张卡**被替换为正文 + 折叠的引用区 | 日志 `处理提问`（有）→ `技能命中：knowledge_qa`；只有"降级"→ 看 RAG 日志；占位卡一直不替换 → 看是否有 `最终卡片更新失败` |
