/** 按账号隔离的 localStorage 读写（问答记录隔离）。
 *
 * 问题：聊天记录若存在全局 key（`chatHistory` / `extractHistory`）下，同一台浏览器上
 * 任何账号打开都是同一份记录——根因是存储 key 没有账号维度，而不是读取时机问题。
 *
 * 做法：key 拼上账号 id（`chatHistory:u3`）。账号未加载时（未登录、或 store 还没拉到
 * userInfo）退回不带账号维度的 key，不会因为拿不到 uid 就把记录写丢。
 *
 * 老数据的处置：历史遗留的那份全局记录不属于任何账号，谁先登录都能看到。这里**不做归属**
...
/**
 * 写 localStorage，返回是否真的写进去了。
 *
 * 必须把成败交回给调用方：若这一层返回 void 且吞掉一切异常，**调用方的 catch
 * 永远不会执行**——`useChatHistory.save()` 里那套"配额不够就丢掉一半会话重试、
 * 还失败就告警"的降级逻辑就成了死代码，localStorage 写满后聊天记录静默停止保存，
 * 刷新即全丢，界面上没有任何提示。
 *
 * 所以**能不能写**是调用方要决定的事（重试、丢弃旧数据、还是提示用户），
