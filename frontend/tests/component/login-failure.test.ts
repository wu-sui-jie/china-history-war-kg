/** 登录页的失败可见性。
 *
 * 被测行为：后端超限返回 **HTTP 429**，axios 对非 2xx 走 reject——
 * 拒绝分支必须把服务端文案显示出来，否则用户点完登录**界面上什么都没有**，
 * 按钮只是重新亮起来（看起来像"点了没反应"，真实原因却是"尝试过于频繁"）。
 *
...
describe('登录页失败可见性', () => {
  ...
  test('登录不做口令长度校验：短口令的老账号必须还能登录', async () => {
    // 老账号的口令可能只有 6~9 位，服务端 authentication 从不检查长度，
  ...
  test('口令按原值发送，不被 trim', async () => {
    // 不能写成 `password: loginForm.password.trim()`：那等于静默改了用户输入。
